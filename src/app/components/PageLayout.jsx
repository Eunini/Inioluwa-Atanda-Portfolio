    ...
    <div className="min-h-screen bg-black text-white">
        ...
        <div className="max-w-[1200px] mx-auto px-6 sm:px-10 h-14 flex items-center justify-between">
          ...
          <div className="hidden md:flex items-center gap-1">
            ...
            <a
              ...
              className="text-xs px-4 py-1.5 rounded-full border border-cyan-500/20 text-cyan-400/70 hover:bg-cyan-500/10 transition-all"
            >
              Book a Session
            </a>
          </div>
              ...
              <div className="px-6 py-4 space-y-1">
                {navLinks.map((link) => (
                  <Link
                    ...
                    className="block text-sm text-white/40 hover:text-white py-2 transition-colors"
                  >
                    {link.name}
                  </Link>
                ))}
                <div className="pt-3 mt-3 border-t border-white/[0.04]">
                  <a
                    ...
                    rel="noopener noreferrer"
                    onClick={() => setMenuOpen(false)}
                    className="block text-sm py-2 text-cyan-400/70"
                  >
                    Book a Session
                  </a>
                </div>
      ...
      <footer className="border-t border-white/[0.04] py-8 px-6 sm:px-10">
        <div className="max-w-[1200px] mx-auto flex flex-col sm:flex-row items-center justify-between gap-4">
          <p className="text-xs text-white/15">Copyright {new Date().getFullYear()} Inioluwa Atanda</p>
